...
func (rq *RoomQuery) QueryCheckout3() *CheckoutQuery {
...
func (rq *RoomQuery) WithCheckout3(opts ...func(*CheckoutQuery)) *RoomQuery {
	query := &CheckoutQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withCheckout3 = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	...
	if query := rq.withCheckout3; query != nil {
		...
		query.Where(predicate.Checkout(func(s *sql.Selector) {
			s.Where(sql.InValues(room.Checkout3Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.room_checkout3
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "room_checkout3" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_checkout3" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Checkout3 = n
		}
	}
