...
func (cq *CustomerQuery) QueryCheckout1() *CheckoutQuery {
...
func (cq *CustomerQuery) WithCheckout1(opts ...func(*CheckoutQuery)) *CustomerQuery {
	query := &CheckoutQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCheckout1 = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withCheckout1; query != nil {
		...
		query.Where(predicate.Checkout(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.Checkout1Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.customer_checkout1
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "customer_checkout1" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_checkout1" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Checkout1 = append(node.Edges.Checkout1, n)
		}
	}
