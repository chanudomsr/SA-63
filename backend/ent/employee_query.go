...
func (eq *EmployeeQuery) QueryCheckout2() *CheckoutQuery {
...
func (eq *EmployeeQuery) WithCheckout2(opts ...func(*CheckoutQuery)) *EmployeeQuery {
	query := &CheckoutQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withCheckout2 = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	...
	if query := eq.withCheckout2; query != nil {
		...
		query.Where(predicate.Checkout(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.Checkout2Column, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.employee_checkout2
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "employee_checkout2" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "employee_checkout2" returned %v for node %v`, *fk, n.ID)
			}
